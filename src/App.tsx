  ...
  return (
    <>
      ...
      <main className="p-4">
        ...
        <FormRide onSubmit={handleSubmit} />

        <button
          className="rounded-lg text-base px-3 w-fullrounded-lg py-2 h-12 uppercase font-medium w-full bg-container-bg text-accent-bg border border-accent-bg my-7"
          onClick={() => {
            loadRides();
          }}
        >
        ...
        <ul className="flex flex-col gap-4 p-0 list-none">
          {rides.map((ride) => (
            <Card ride={ride} onDelete={handleDelete} key={ride.id} />
          ))}
        </ul>
      </main>
    </>
